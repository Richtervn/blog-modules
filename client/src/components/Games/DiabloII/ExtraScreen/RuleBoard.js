...
export default ({ data }) => (
  <div className="d2-extra-rule-board">
    <div className="text-center">
      <h5>Saving characters will increase next created characters' stats</h5>
      <div>
        <b>Extra levels from non-saved character will be calculated by the following rules :</b>
      </div>
    </div>
    <ul style={{ marginTop: '15px' }}>{getRules(data).map((rule, i) => <li key={i}>{rule}</li>)}</ul>
  </div>
);
