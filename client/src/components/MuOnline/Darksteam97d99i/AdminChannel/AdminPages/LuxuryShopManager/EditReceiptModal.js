...
class ModalAddReceipt extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		const nextState = {
			...
			name: receipt.name,
			price: receipt.price,
			charge_price: receipt.charge_price,
			description: receipt.description,
			category: receipt.category,
			itemId: receipt.itemId,
			luck: receipt.luck == 1,
			skill: receipt.skill == 1,
			option: receipt.option,
			duration: receipt.duration,
			level: receipt.level,
			slot: receipt.slot,
			exc1: receipt.exc1 == 1 ? true : false,
			exc2: receipt.exc2 == 1 ? true : false,
			exc3: receipt.exc3 == 1 ? true : false,
			exc4: receipt.exc4 == 1 ? true : false,
			exc5: receipt.exc5 == 1 ? true : false,
			exc6: receipt.exc6 == 1 ? true : false,
			materials: receipt.materials.map(item => {
				return {
					id: item.id,
					name: item.name,
	...
	render() {
		...
		return (
			<div className="modal fade" id="editDs9799LxReceiptModal" style={{ color: 'black' }}>
