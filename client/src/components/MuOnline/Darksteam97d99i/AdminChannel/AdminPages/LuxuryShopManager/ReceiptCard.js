...
export default ({ admin, receipt, onClickEdit, onClickDelete, onClickBuy }) => {
	return (
		<div className="col-3 no-col-margin">
			<div className="ds9799-lx-receipt-card">
				...
				<img src={receipt.image_url.replace('./public', 'http://localhost:3000')} style={{ paddingBottom: '10px' }} />
				...
				<div className="ds9799-lx-receipt-describle text-center" style={{width: '100%'}}>
					...
					{receipt.materials.map(material => (
						<div key={material.id}>
							{material.count} x <img src={defineItemImage(material.category, material.itemId, material.level)} />&nbsp;
							{material.name}
						</div>
					))}
