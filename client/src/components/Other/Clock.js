...
export default class Clock extends Component {
  static defaultProps = {
    size: 250, // size in pixels => size is length & width
  ...
  drawFace(ctx, radius) {
    ...
    const grad = ctx.createRadialGradient(0, 0, radius * 0.9, 0, 0, radius * 1.2);
  ...
  drawTime(ctx, radius) {
    ...
    this.drawHand(ctx, hour, radius * 0.45, radius * 0.05);
    // minute
    minute = (minute * Math.PI) / 30 + (second * Math.PI) / (30 * 60);
    this.drawHand(ctx, minute, radius * 0.7, radius * 0.05);
    // second
    second = (second * Math.PI) / 30;
    this.drawHand(ctx, second, radius * 0.95, radius * 0.02, 'red');
  }
