...
class BagItemsEditor extends Component {
  ...
  generateLine({ category, itemId, level, skill, luck, option }) {
  ...
  generate() {
    const { category, itemId, level, skill, luck, option } = this.state;
  ...
  async generateFile() {
    ...
    let fileName = _.findWhere(EventItemBagList, { Name: fileData[3].replace('// ', '') }).File;
