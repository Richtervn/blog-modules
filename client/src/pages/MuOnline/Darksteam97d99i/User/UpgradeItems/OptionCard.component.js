...
import { getExcText, getItemOptions } from 'helpers/mu';
...
class OptionCard extends Component {
  ...
  render() {
    ...
    if (!item)
      return (
        <div id="ds9799-option-card" className="notice">
          <h5>
    ...
    const { excOpt1, excOpt2, excOpt3, excOpt4, excOpt5, excOpt6 } = getExcText(item.category, item.itemId);

    const { exc1, exc2, exc3, exc4, exc5, exc6, luck, skill } = this.state.value;

    return (
      <div id="ds9799-option-card">
        <div className="container">
          ...
          <div className="price-btn">
            <button className="btn btn-danger btn-block" onClick={this.handleUpgrade}>
              Upgrade : <span>{formatNumber(this.state.price)}</span> credits
            </button>
          </div>
        </div>
      </div>
